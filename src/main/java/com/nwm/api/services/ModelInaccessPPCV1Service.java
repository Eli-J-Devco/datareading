...
@Service
public class ModelInaccessPPCV1Service extends DB {
	/**
	...
	public ModelInaccessPPCV1Entity setModelInaccessPPCV1(String line) {
		try {
			List<String> words = Lists.newArrayList(Splitter.on(',').split(line));
			if (words.size() > 0) {
				ModelInaccessPPCV1Entity dataModel = new ModelInaccessPPCV1Entity();
				...
				dataModel.setDIGITAL_OUTPUT_P_CTRL_SET_RELEASE(Double.parseDouble(!Lib.isBlank(words.get(26)) ? words.get(26) : "0.001"));
				dataModel.setDIGITAL_OUTPUT_PF_CTRL_SET(Double.parseDouble(!Lib.isBlank(words.get(27)) ? words.get(27) : "0.001"));
				dataModel.setDIGITAL_OUTPUT_Q_CTRL_SET(Double.parseDouble(!Lib.isBlank(words.get(28)) ? words.get(28) : "0.001"));
				dataModel.setDIGITAL_OUTPUT_Q_CTRL_SET_RELEASE(Double.parseDouble(!Lib.isBlank(words.get(29)) ? words.get(29) : "0.001"));
				
				return dataModel;
				
			} else {
				return new ModelInaccessPPCV1Entity();
			}
	...
	public boolean insertModelInaccessPPCV1(ModelInaccessPPCV1Entity obj) {
		try {
			Object insertId = insert("ModelInaccessPPCV1.insertModelInaccessPPCV1", obj);
	        if(insertId == null ) {
	        	return false;
	        }
	        return true;
